	...
	<div class="app-container">
		...
		<main class="main-content">
			...
			<script>
				...
				window.addEventListener('DOMContentLoaded', () => {
					// ページ読み込み時に100%のものを一番下に整理
					reorderCompletedProjects();
				...
				// ▼▼▼ 100%完了したプロジェクトを自動で一番下に沈める関数 ▼▼▼
				function reorderCompletedProjects() {
					const listContainer = document.getElementById("projectList");
					...
					cards.sort((a, b) => {
						...
						const is100A = progA === 100 ? 1 : 0;
						const is100B = progB === 100 ? 1 : 0;
						
						// 100%のプロジェクトを後ろに移動する
						if (is100A !== is100B) {
							return is100A - is100B;
						}
						return 0;
					});
				...
				function sortProjects() {
					...
					cards.sort((a, b) => {
						// 優先して100%完了のものは一番下にしたい場合はここで調整可能ですが、
						// 基本のソートを適用したあとに再整理することもできます
						if (sortType === "newest") {
							return parseInt(b.getAttribute("data-raw-id")) - parseInt(a.getAttribute("data-raw-id"));
						} else if (sortType === "name") {
							const nameA = a.getAttribute("data-name");
					...
					// 最後に100%完了のものは強制的に一番下にしたい場合は以下のコメントを外してください
					// reorderCompletedProjects();
				}
				...
				function calculateProgressLocal() {
					...
					if (barFill) {
						...
						// 進捗更新に伴い、100%になったプロジェクトを一番下に再配置
						reorderCompletedProjects();
					}
				}
			</script>
